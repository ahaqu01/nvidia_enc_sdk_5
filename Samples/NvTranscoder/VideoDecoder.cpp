...
CudaDecoder::CudaDecoder() : m_bFinish(false), m_decodedFrames(0),
    m_videoSource(NULL), m_videoParser(NULL), m_videoDecoder(NULL), m_ctxLock(NULL)
{
}
        ...
        int targetWidth, int targetHeight)
{
    ...
    CUVIDEOFORMAT& oFormat = oFormatEx.format;
    cuvidGetSourceVideoFormat(m_videoSource, &oFormat, CUVID_FMT_EXTFORMATINFO);

    if (oFormat.progressive_sequence == 0) {
        fprintf(stderr, "The sample only supports progressive video!\n");
        exit(-1);
    }
    ...
    // parse SPS data by custom
    // profile_idc is in oFormatEx.raw_seqhdr_data[4]
    int profile = oFormatEx.raw_seqhdr_data[4];
    if (profile != 66 && profile != 77 && profile != 100) {
        fprintf(stderr, "The sample only supports Baseline/Main/High profile H264, yours is %s\n", getProfileName(profile));
        exit(-1);
    }
