...
#ifdef ZYHCODE
    encodeConfig.width = 1920;
    encodeConfig.height = 1080;
    encodeConfig.inputFileName = "../YUV/1080p/HeavyHandIdiot.3sec.yuv";
    encodeConfig.outputFileName = "output.avi";
   // uint8_t* yuv_img_buff= (uchar*)malloc(sizeof(uchar)*rgb_img.rows*rgb_img.cols*3/2);
#endif
...
printf("after nvStatus222222\n");
    ...
    NvQueryPerformanceCounter(&lStart);
printf("after NvQueryPerformanceCounter\n");
#ifdef READ_LOCAL_VIDEO
    for (int frm = encodeConfig.startFrameIdx; frm <= encodeConfig.endFrameIdx; frm++)
    {
       numBytesRead = 0;
   ...
   // cout<<"帧率="<<frameRate<<endl;
    ...
    //cvtColor(frame, yuvImg, CV_BGR2YUV_I420);
	 Mat frame;
        cap>>frame;
    //uint8_t* yuv_img_buff= (uchar*)malloc(sizeof(uint8_t)*frame.rows*frame.cols*3/2);
    uchar *yuv_img_buff[3];
	yuv_img_buff[0]=new uchar[sizeof(uchar)*frame.rows*frame.cols];
        yuv_img_buff[1]=new uchar[sizeof(uchar)*frame.rows/2*frame.cols/2];
        yuv_img_buff[2]=new uchar[sizeof(uchar)*frame.rows/2*frame.cols/2];

   for(int count = 0; count<10000;count++)
    {

#ifdef NO_GPU_RGB2YUV
	 int yuv_size = width*height*3/2;

	 memcpy(yuv[0], yuvImg.data, width*height);
	 memcpy(yuv[1], yuvImg.data+width*height, width*height/4);
	 memcpy(yuv[2], yuvImg.data+width*height*5/4, width*height/4);
        EncodeFrameConfig stEncodeFrame;
        memset(&stEncodeFrame, 0, sizeof(stEncodeFrame));
        stEncodeFrame.yuv[0] = yuv[0];
        stEncodeFrame.yuv[1] = yuv[1];
	 ...
	 rgb2yuv(frame, yuv_img_buff);
	 cout<<"count = "<<count<<endl;
       EncodeFrameConfig stEncodeFrame;
        memset(&stEncodeFrame, 0, sizeof(stEncodeFrame));

	memcpy(yuv[0],yuv_img_buff[0],width*height);
	memcpy(yuv[1],yuv_img_buff[1],width*height/4);
	 memcpy(yuv[2],yuv_img_buff[2],width*height/4);
        stEncodeFrame.yuv[0] = yuv_img_buff[0];
        stEncodeFrame.yuv[1] = yuv_img_buff[1];
        stEncodeFrame.yuv[2] = yuv_img_buff[2];
...
int CNvEncoder::initNvEncoder(int weight, int height, EncodeConfig &encodeConfig, char *videoName)
{
    NVENCSTATUS nvStatus = NV_ENC_SUCCESS;
    ...
    encodeConfig.pictureStruct = NV_ENC_PIC_STRUCT_FRAME;
    encodeConfig.isYuv444 = 0;
    encodeConfig.width = weight;
    encodeConfig.height = height;
    encodeConfig.inputFileName = "../YUV/1080p/HeavyHandIdiot.3sec.yuv";
    encodeConfig.outputFileName = videoName;
    encodeConfig.fOutput = fopen(encodeConfig.outputFileName, "wb");
    if (encodeConfig.fOutput == NULL)
    {
        PRINTERR("Failed to create \"%s\"\n", encodeConfig.outputFileName);
...
int CNvEncoder::EncodeH264(uchar *yuv_img_buff[3], EncodeConfig encodeConfig)
{
	EncodeFrameConfig stEncodeFrame;
	memset(&stEncodeFrame, 0, sizeof(EncodeFrameConfig));
	stEncodeFrame.yuv[0] = yuv_img_buff[0];
	stEncodeFrame.yuv[1] = yuv_img_buff[1];
	stEncodeFrame.yuv[2] = yuv_img_buff[2];
//cout<<"sizeof(yuv_img_buff[0]) = "<<yuv_img_buff[0]<<endl;
	stEncodeFrame.stride[0] = encodeConfig.width;
	stEncodeFrame.stride[1] = encodeConfig.width / 2;
	stEncodeFrame.stride[2] = encodeConfig.width / 2;
	stEncodeFrame.width = encodeConfig.width;
...
#endif
uchar *yuv_img_buff[3];
int MatToH264Video(Mat &frame, bool &isNewVideo, bool &isLastFrame, char *videoName)
{
    ...
    {
	 NvQueryPerformanceCounter(&lStart);
        nvEncoder.initNvEncoder(frame.cols, frame.rows, encodeConfig, videoName);
        yuv_img_buff[0]=new uchar[sizeof(uchar)*frame.rows*frame.cols];
        yuv_img_buff[1]=new uchar[sizeof(uchar)*frame.rows/2*frame.cols/2];
        yuv_img_buff[2]=new uchar[sizeof(uchar)*frame.rows/2*frame.cols/2];
		count = 0;
	 ...
	 cout<<"before rgb2yuv---"<<endl;
        rgb2yuv(frame, yuv_img_buff);
        nvEncoder.EncodeH264(yuv_img_buff, encodeConfig);
	if(isLastFrame)
	{
		isLastFrame = false;
	    ...
	    for (int i = 0; i < 3; i ++)
	    {
	        if (yuv_img_buff[i])
	        {
	            delete [] yuv_img_buff[i];
	        }
	    }
	    nvStatus = nvEncoder.EncodeFrame(NULL, true, encodeConfig.width, encodeConfig.height);
		cout<<"after encodeFrame---"<<endl;
	    if (nvStatus != NV_ENC_SUCCESS)
	    {
	        bError = true;
	        goto exit;
	    ...
	    if (count > 0)
	    {
	        ...
	        NvQueryPerformanceFrequency(&lFreq);
	        double elapsedTime = (double)(lEnd - lStart);
	        printf("Encoded %d frames in %6.2fms\n", count, (elapsedTime*1000.0)/lFreq);
	        printf("Avergage Encode Time : %6.2fms\n", ((elapsedTime*1000.0)/count)/lFreq);
	    }
	}
...
exit:
    ...
    nvEncoder.Deinitialize(encodeConfig.deviceType);
...
int main(int argc, char **argv)
{
	...
	{
		if(i==2000)isLastFrame = true;
		MatToH264Video(frame, isNewVideo,  isLastFrame, videoName);
...
int main(int argc, char **argv)
{
    ...
    cout<<"before free(yuv_img_buff)---"<<endl;
    for (int i = 0; i < 3; i ++)
    {
        if (yuv_img_buff[i])
        {
	...
	system("ffmpeg -i output2.avi -vcodec copy -f mp4 output2.mp4");
    if (count > 0)
    {
        NvQueryPerformanceCounter(&lEnd);
        NvQueryPerformanceFrequency(&lFreq);
